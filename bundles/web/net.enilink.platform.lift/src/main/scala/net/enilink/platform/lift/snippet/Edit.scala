...
import net.enilink.komma.edit.domain.{IEditingDomain, IEditingDomainProvider}
import net.enilink.komma.edit.properties.{EditingHelper, IEditingSupport, IResourceProposal}
...
import net.liftweb.http.{DispatchSnippet, ParsePath, S}
...
class JsonCallHandler {
  ...
  def model: Box[IModel] = Globals.contextModel.vend

  val path: Box[ParsePath] = S.request map (_.path)

  class RdfEditingHelper(editType: EditingHelper.Type) extends EditingHelper(editType) {
    override def getEditingDomain: IEditingDomain = model.map(_.getModelSet.adapters.getAdapter(classOf[IEditingDomainProvider]) match {
      case p: IEditingDomainProvider => p.getEditingDomain
      case _ => null
    }) openOr null

    override def getEditingSupport(element: Object): IEditingSupport = {
      super.getEditingSupport(element)
    }

    override def setProperty(element: Any, property: IProperty) : Unit = {}

    override def execute(command: ICommand): CommandResult = command match {
  ...
  val NULL_URI: URI = URIs.createURI("komma:null")
    ...
    (rdf match {
      ...
      case JString(rdfStr) =>
        ...
        ModelUtil.readData(new ByteArrayInputStream(rdfStr.getBytes("UTF-8")), "", null, true, new IDataVisitor[Unit]() {
          ...
          override def visitStatement(stmt: IStatement): Unit = stmts += stmt
        })
        stmts.toSeq
      case _ => Nil
    }) map {
  ...
  def renameNewNodes(stmts: Seq[IStatement], em: IEntityManager, baseURI: URI, replacements: mutable.Map[String, IReference] = new mutable.HashMap): Seq[IStatement] = {
