...
m.html;s.lineIndex=o;s.style.height=n+p.top+"px"}var q;if(i==="page"){for(h=h.getAnnotations(g,c+1);t;)o=t.lineIndex,m=t.nextSibling,(!(g<=o&&o<=c)||t.lineChanged)&&a.removeChild(t),t=m;t=a.firstChild.nextSibling;q=k.createDocumentFragment();for(o=g;o<=c;o++)if(!t||t.lineIndex>o){s=b.createElement(k,"div");if(m=h[o]){f(m.style,s);if(m.html)s.innerHTML=m.html;s.annotation=m}s.lineIndex=o;s.style.height=this._getLineHeight(o)+"px";q.appendChild(s)}else if(q.firstChild&&(a.insertBefore(q,t),q=k.createDocumentFragment()),
t)t=t.nextSibling;q.firstChild&&a.insertBefore(q,t)}else{m=this._getClientHeight();o=this._model.getLineCount();t=m+p.top+p.bottom-2*this._metrics.scrollWidth;i=n*o<t?n:t/o;if(a.rulerChanged){for(m=a.childNodes.length;m>1;)a.removeChild(a.lastChild),m--;h=h.getAnnotations(0,o);q=k.createDocumentFragment();for(var v in h)if(o=v>>>0,!(o<0)){s=b.createElement(k,"div");m=h[v];f(m.style,s);s.style.position="absolute";s.style.top=this._metrics.scrollWidth+n+Math.floor(o*i)+"px";if(m.html)s.innerHTML=m.html;
s.annotation=m;s.lineIndex=o;q.appendChild(s)}a.appendChild(q)}else if(a._oldTrackHeight!==t)for(s=a.firstChild?a.firstChild.nextSibling:null;s;)s.style.top=this._metrics.scrollWidth+n+Math.floor(s.lineIndex*i)+"px",s=s.nextSibling;a._oldTrackHeight=t}a.rulerChanged=!1;a=a.nextSibling}},_updateStyleSheet:function(){var a="";b.isWebkit&&this._metrics.scrollWidth>0&&(a+="\n.textview ::-webkit-scrollbar-corner {background: #eeeeee;}");b.isFirefox&&b.isMac&&this._highlightRGB&&this._highlightRGB!=="Highlight"&&
